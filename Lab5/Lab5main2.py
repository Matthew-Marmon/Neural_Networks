...
L3 = DropoutLayer(0)
L4 = TanhLayer()
L5 = FullyConnectedLayer(hidden_dim, 1)
L6 = DropoutLayer(0)
L7 = LogisticSigmoidLayer()
L8 = LogLoss()
...
L5.setWeights(np.random.uniform(-init2, init2, (hidden_dim, 1)))
L5.setBiases(np.random.uniform(-init2, init2, (1, 1)))
...
while epoch < max_epoch:
    ...
    for L in range(len(Layers)-2, 0, -1):
        ...
        grad = Layers[L].backward2(grad)
    #test
    H = Layers[0].forward(Xtest)  # Normalize test data using training stats
    for L in range(1, len(Layers)-1):
        if not isinstance(Layers[L], DropoutLayer):
            H = Layers[L].forward(H)
    Yhat_test = H
